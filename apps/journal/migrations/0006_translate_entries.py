...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for language_code, language_name in settings.LANGUAGES:
            for entry in orm['journal.Entry'].objects.all():
                prefix = '' if language_code == settings.LANGUAGE_CODE else "TRANSLATE INTO " + language_code.upper() + " - "

                translation = orm['journal.EntryTranslation'].objects.create(
                    language_code=language_code,
                    master=entry,
                    t_title=prefix + entry.title,
                    t_content=("<p>" + prefix + "</p>" if prefix else "") + entry.content,
                    t_excerpt=prefix + entry.excerpt,
                )

    def backwards(self, orm):
        orm['journal.EntryTranslation'].objects.all().delete()
    ...
    models = {
        ...
        'journal.entry': {
            ...
            'pub_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2011, 9, 29, 18, 55, 41, 938362)'}),
